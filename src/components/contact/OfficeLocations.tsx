...
export default function OfficeLocations() {
  return (
    <div className="relative rounded-2xl border border-navy-950/[0.07] bg-mist-100/40 backdrop-blur-sm p-8 overflow-hidden">
      {/* Ambient glow */}
      ...
      <h3 className="text-2xl font-bold text-navy-950 font-heading mb-7 relative">
        Office Address
      </h3>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 relative">
        {offices.map((office) => (
          <div
            key={office.country}
            className="group flex items-start gap-4 rounded-xl border border-navy-950/[0.07] bg-mist-100/60 p-5 transition-all duration-300 hover:border-teal-500/25 hover:bg-mist-100"
          >
            <span className="shrink-0 w-11 h-11 rounded-lg overflow-hidden border border-navy-950/10 shadow-sm">
              <img
            ...
            <div>
              <div className="flex flex-wrap items-center gap-x-2 gap-y-1 mb-1.5">
                <span className="text-navy-950 font-semibold text-sm">{office.country}</span>
