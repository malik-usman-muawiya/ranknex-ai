  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold font-heading text-navy-950">Manage Case Studies</h1>
          <p className="text-slate-500 text-sm mt-1">Manage, showcase, and publish customer success metrics.</p>
        </div>
      ...
      {/* Filters bar */}
      <div className="grid grid-cols-1 sm:grid-cols-12 gap-4 bg-white border border-slate-200/80 p-4 rounded-2xl shadow-xs">
        {/* Search */}
                ...
                filteredStudies.map((study) => (
                  <tr key={study.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="py-4 px-6 max-w-xs sm:max-w-sm lg:max-w-md">
                      <div className="font-bold text-navy-950 line-clamp-1">{study.title}</div>
                      <div className="text-xs text-slate-500 mt-0.5 truncate max-w-xs">{study.metrics}</div>
                    </td>

                    {/* Industry */}
                    <td className="py-4 px-6 font-medium text-slate-600">{study.clientIndustry}</td>
